...
export default function WeatherDay ({today, data, selected, setSelected}) {
    ...
    const { date, maxtemp_f, mintemp_f, avgtemp_f, maxwind_mph, totalprecip_in, avgvis_miles, avghumidity, daily_will_it_rain, daily_chance_of_rain, daily_will_it_snow, daily_chance_of_snow, condition, uv } = data;
    ...
    return (
        ...
        ${selected === date ? "rounded-md" : "rounded-2xl"} transition-all duration-300 border-black border-solid p-8 text-2xl shadow-2xl`}>
            {today ? "Today" :<Date dateString={date} />}
            <div className="flex justify-between items-center">
                <p className="text-lg">{avgtemp_f + degreeSymbol}</p>
